...
from production_rag.server.chunks.chunks_service import Chunk
...
class ChatService:
    ...
    def __init__(
        ...
        self.rag_pipeline = RagPipeline(self.index, self.llm_component.llm)
    ...
    def chat(
        ...
        if use_context:
            chat_engine_input = ChatEngineInput.from_messages(messages)
            query = chat_engine_input.last_message.content if chat_engine_input.last_message else ""
            
            # Use the new production-grade RAG pipeline
            result = self.rag_pipeline.run(query)
            
            sources = [
                Chunk(
                    content="", # content is summarized in the answer but we can provide objects if needed
                    file_name=src["file_name"],
                    page=src["page"],
                    score=src["score"]
                ) for src in result["sources"]
            ]
            return Completion(response=result["answer"], sources=sources)
